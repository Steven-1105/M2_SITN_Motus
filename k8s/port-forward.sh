...
#  EXACTEMENT comme docker-compose. Le front (http://localhost:5500)
...
( cd "$(dirname "$0")/../frontend" && python3 -m http.server 5500 >/tmp/motus-front.log 2>&1 & )
echo "Front sur http://localhost:5500  |  services sur 8081-8084"

trap 'echo; echo "Arret des tunnels…"; kill $(jobs -p) 2>/dev/null; pkill -f "http.server 5500" 2>/dev/null' INT
wait
